...
function gerarCodigo() {
  ...
  // Cria um objeto com os dados das opções criadas
  const dados = { quantidade: opcoes.length, opcoes: [] };
  // Percorre cada opção e salva suas informações
  opcoes.forEach((opcao, i) => {
  ...
  // Gera um código aleatório de 6 caracteres, SÓ 6 NADA MAIS
  const codigo = Math.random().toString(36).substr(2, 6).toUpperCase();
  localStorage.setItem(codigo, JSON.stringify(dados));
  localStorage.setItem("codigo_atual", codigo);
  // Mostra o código gerado na tela embaixo dos botões
  document.getElementById("codigo").innerHTML = `Código: <strong>${codigo}</strong>`;
  document.getElementById("qrcode").innerHTML = "";
  // Gera um QR Code para acessar a página
  ...
  localStorage.setItem("respostas_ativas_" + codigo, "true");  // libera as respostas para o aluno
...
document.getElementById("btn-monitorar").addEventListener("click", () => {
  ...
  window.location.href = `professor_mon.html?codigo=${codigo}`;
});
